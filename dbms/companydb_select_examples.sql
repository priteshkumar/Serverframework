...
select ssn,fname || ' ' || minit || ' ' || lname as fullname
from employee
...
--- categorise project wise member count in increasing order 
select pname,count(essn)
from works_on
inner join project on works_on.pno = project.pnumber
group by pname
order by count(essn) asc 
...
--- hardcoding emp ssn for depdendents having at least 1 dependent
select avg(salary)
from employee
where ssn = '123456789' or ssn = '987654321' or ssn = '333445555'


-- employee wise dependent distribution
select essn,count(*)
from dependent 
group by essn
having count(*) >= 1
...
select e.ssn,d.dname,p.pname,dp.dname
from employee e inner join department d on e.dno = d.dnumber
inner join works_on w on e.ssn = w.essn
inner join project p on w.pno = p.pnumber
inner join department dp on p.dnum = dp.dnumber
...
group by d.dname
having count(ssn) = (select min(mycount) from mytable)
...
--     average salary?
	...
	inner join department d on e.dno = d.dnumber
	group by d.dname
	having avg(salary) = (select max(avgsalary) from salarystats) 
...
-- self join example to query hierarchal data
...
select e.fname as "EmpName",m.fname as "MgrName"
from employee e 
...
select e.ssn,p.pname,d.dname,pd.dname
from employee e inner join department d on e.dno = d.dnumber
inner join works_on w on e.ssn = w.essn
inner join project p on w.pno = p.pnumber
...
--- test queries
-- Write the SQL query to find the no of male and female employees in the 'company' database.
select sex,count(*) from 
employee
group by sex


-- Write the SQL query to get the department no and the total salary with respect to a department from 'employee' table.
select dno,sum(salary) as "Total Salary"
from employee
group by dno


-- Write the SQL query to find the average salaries of both male and female employees in the 'company' database.
select sex, avg(salary) as "Average Salary"
from employee
group by sex



-- Write the SQL query to find all the departments whose average salary is greater than 30000 in the 'employee' table.
...
-- average salary departmentwise/genderwise
select dno,sex,avg(salary) as "Average Salary"
from employee
group by dno,sex
